...
function cps_font_url() {
	 ...
	 */
	$roboto = _x( 'on', 'Roboto font: on or off', 'cps' );
	$open_sans = _x( 'on', 'Open Sans font: on or off', 'cps' );
...
function cps_scripts() {
	...
	// Register styles.
	wp_register_style( 'cps-google-font', cps_font_url(), array(), null );
	...
	// Enqueue scripts.
	wp_enqueue_script( 'cps-scripts', get_template_directory_uri() . '/assets/scripts/project' . $suffix . '.js', array( 'jquery' ), $version, true );
	...
	wp_enqueue_script( 'cps-mobile-nav', get_template_directory_uri() . '/assets/scripts/mobile-nav-menu' . $suffix . '.js', array( 'jquery' ), $version, true );
}
 ...
 */
function cps_include_svg_icons() {
...
add_action( 'wp_footer', 'cps_include_svg_icons', 9999 );
